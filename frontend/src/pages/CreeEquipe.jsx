...
const CreeEquipe = ({ setCurrentView, structure, setSelectedEquipe }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!structureId) {
      setError("Structure parente introuvable. Veuillez repasser par le menu de gestion de structure.");
      return;
    }
    ...
    try {
      ...
      const response = await API.post('/r_equipe', { 
        nom: nom.trim(), 
        structureId: structureId 
      });
      
      setSelectedEquipe(response.data);
      setSuccess("L'équipe a été créée avec succès ! Redirection...");
      
      setTimeout(() => { 
        setCurrentView('manage-equipe'); 
      }, 2000);
    } catch (err) {
      setError(err.response?.data?.error || "Une erreur est survenue lors de la création de l'équipe.");
    } finally {
      setLoading(false);
    }
  };
        ...
        <div style={{ width: '100%', maxWidth: '750px', display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', width: '100%', borderBottom: '1px solid var(--border-color)', paddingBottom: '1rem' }}>
            <div style={{ textAlign: 'left' }}>
              <h2 style={{ fontSize: '1.6rem', fontWeight: '700', color: 'var(--text-main)', margin: '0 0 0.4rem 0' }}>Créer une équipe</h2>
              <p style={{ fontSize: '0.9rem', color: 'var(--text-muted)', margin: 0 }}>Initialisez une nouvelle division sportive au sein de votre organisation. Vous y serez rattaché en tant que manager par défaut.</p>
            </div>
            
            <div style={{ display: 'flex', gap: '0.75rem' }}>
              <button type="button" onClick={() => setCurrentView('manage-structure')} className="btn-secondary">← Retour</button>
          ...
          <div className="settings-card">
            <div className="settings-row">
              <div className="settings-row-header">
                <div className="settings-label wide">NOM DE L'ÉQUIPE *</div>
                <div className="settings-value">
                  <input 
                    type="text" 
                    placeholder="Ex: Équipe Senior A" 
                    value={nom} 
